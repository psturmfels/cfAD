...
float update_gradients(float * const u_i_ptr, float * const v_j_ptr, float e_ij, float eta, float lamb, int k, std::vector<float> neighbor_average, float lamb2) 
{
    for (int l = 0; l < k; ++l) 
    {
        ...
        *(u_i_ptr + l) += eta * (e_ij * v_jl - lamb * u_il + lamb2 * neighbor_average[l]);
        *(v_j_ptr + l) += eta * (e_ij * u_il - lamb * v_jl + lamb2 * neighbor_average[l]);
    }
}

float update_gradients(float * const u_i_ptr, float * const v_j_ptr, float e_ij, float eta, float lamb, int k) 
{
    for (int l = 0; l < k; ++l) 
    {
        float u_il = *(u_i_ptr + l);
        ...
        *(u_i_ptr + l) += eta * (e_ij * v_jl - lamb * u_il);
        *(v_j_ptr + l) += eta * (e_ij * u_il - lamb * v_jl);
    }
}
...
void Factor(np::ndarray X, np::ndarray U, np::ndarray V, np::ndarray known_indices, p::dict neighbors, float eta=0.005, float lamb=0.02, float lamb2=0.0001, int num_epochs=200)
{
    int numKnown = known_indices.shape(0);
    ...
    for (int epoch = 0; epoch < num_epochs; ++epoch) 
    {
        ...
        for (int known_ind = 0; known_ind < numKnown; ++known_ind) 
        {
            ...
            int i = p::extract<int>(known_indices[rand_ind][0]);
            int j = p::extract<int>(known_indices[rand_ind][1]);
            ...
            if (neighbors.has_key(j))
            {
                ...
                update_gradients(u_i_ptr, v_j_ptr, e_ij, eta, lamb, k, neighbor_average, lamb2);
            }
            
            update_gradients(u_i_ptr, v_j_ptr, e_ij, eta, lamb, k);
        }
        float error = Get_prediction_error(X, U, V, known_indices);
        std::cout << "Epoch " << epoch << ", current training error: " << error << std::endl;
    }
    return;
}
